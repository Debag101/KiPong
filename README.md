# KiPong
KiPong is a simple recreation of the classic Pong Game made by Atari, Inc. in 1972. 

## Images
![Loading Screen](../resources/screenshots/loading_scree.png)
![Gameplay](../resources/screenshots/Gameplay.png)
![Ending Screen](../resources/screenshots/ending_scree.png)

## How to play the game

### Prerequisites 
- Python: Any `python` version above python `3.11` would be preferred
- Pip : `Pip` is needed to handle and install kivy's dependencies
- Graphics: Need a GPU with atleast `OpenGL (ES) 2.0+` to render the UI. For `Non-GPU` users you can use Mesa 3D for `Linux` or 

